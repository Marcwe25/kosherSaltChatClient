...
export async function requestAccessToken (refresh_token) {
    try {
        ...
        const param4 = "Bearer " + refresh_token;
        const data = await axiosRefresh.post(refresh_token_url,{ "token" : param4 })
        const got_access_token = data.access_token;
        const got_refresh_token = refresh_token;
        console.log("got_access_token data")
